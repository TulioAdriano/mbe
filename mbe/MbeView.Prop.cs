...
namespace mbe
{
	partial class MbeView
	{
		private bool EditPropertyText(MbeObjText obj)
		{
            ...
            if (obj.Layer != MbeLayer.LayerValue.DOC) {
                dlg.UseSystemfontOnDoc = SetTextForm.CHK_USE_SYSTEMFONT_ON_DOC_STATE.DISABLED;
            }else {
                dlg.UseSystemfontOnDoc = (obj.UseSystemFontOnDoc ? SetTextForm.CHK_USE_SYSTEMFONT_ON_DOC_STATE.TRUE : SetTextForm.CHK_USE_SYSTEMFONT_ON_DOC_STATE.FALSE);
            }
			...
			if (retv == DialogResult.OK) {
                ...
                if (obj.Layer == MbeLayer.LayerValue.DOC) {
                    obj.UseSystemFontOnDoc = (dlg.UseSystemfontOnDoc != SetTextForm.CHK_USE_SYSTEMFONT_ON_DOC_STATE.FALSE);
                }
                return true;
			}
			return false;

		...
		protected void PlaceOutlines()
        {
            ...
            foreach (MbeObj obj in workList) {
                if (obj.Id() == MbeObjID.MbePTH ||
                    ...
                    if (!obj.ConnectionCheckActive || obj.TempPropString.Length > 0) {
                        int netnum;
                        if (!obj.ConnectionCheckActive) {       //接続されていないときは通常のパターンギャップを使う
                            outlineParam.gap = patternGap;
                            netnum = -1;
                        } else {
                            outlineParam.gap = thermalGap;      //部品のピンのときはサーマルカットを指定する。
                            netnum = 0;
                        }
                ...
                if (obj.Layer == layer || obj.Layer == MbeLayer.LayerValue.PTH) {
                    if (obj.Id() == MbeObjID.MbePTH ||
                        ...
                        if (obj.ConnectionCheckActive && obj.TempPropString.Length > 0) {
                            ...
                            if(obj.Id() == MbeObjID.MbePTH &&(layer == MbeLayer.LayerValue.L2 || layer == MbeLayer.LayerValue.L3)) {
                                padHeight = padWidth = ((MbeObjPTH)obj).InnerLandDia();
                            }else {
                                padWidth = ((MbeObjPin)obj).PadSize.Width;
                                padHeight = ((MbeObjPin)obj).PadSize.Height;
                            }

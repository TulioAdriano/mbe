	...
	class MbeObjPTH : MbeObjPin
	{
        ...
        public override void GenerateOutlineData(LinkedList<MbeGapChkObjLine> outlineList, GenOutlineParam param)
        {
            ...
            if (param.layer == MbeLayer.LayerValue.L2 || param.layer == MbeLayer.LayerValue.L3) {
                ...
                //内層のランド径について 0.53.01 までは非接続のときはランドを置かずにポリゴンを作成し
                //接続時は表層と同サイズのランドを置いた。
                //内層でネストしたポリゴンを配置すると、先に生成したポリゴンに隣接したスルーホールでトラブルが発生した。
                //0.53.02で変更。InnerLandDia の動作を変更。

                int landDia = InnerLandDia();
        ...
        public int InnerLandDia()
        {
            ...
            int maxDia = dia + 8000;
            int surfaceLandSizeSmaller = (padSize.Width < padSize.Height ? padSize.Width : padSize.Height);
            return (maxDia < surfaceLandSizeSmaller ? maxDia : surfaceLandSizeSmaller);
		...
		public override void GenerateCamData(CamOut camOut)
		{
            ...
            if ((innerLayerConnectionInfo & (ulong)MbeLayer.LayerValue.L2) != 0) {
                //camd = new CamOutBaseData(MbeLayer.LayerValue.L2, CamOutBaseData.CamType.FLASH, _shape, innerLandDia, innerLandDia, pt0, pt1);
                camd = new CamOutBaseData(MbeLayer.LayerValue.L2, CamOutBaseData.CamType.FLASH, CamOutBaseData.Shape.Obround, innerLandDia, innerLandDia, pt0, pt1);    //L2は表層ランド形状に関係なくObround
                camOut.Add(camd);
            }

            if ((innerLayerConnectionInfo & (ulong)MbeLayer.LayerValue.L3) != 0) {
                //camd = new CamOutBaseData(MbeLayer.LayerValue.L3, CamOutBaseData.CamType.FLASH, _shape, innerLandDia, innerLandDia, pt0, pt1);
                camd = new CamOutBaseData(MbeLayer.LayerValue.L3, CamOutBaseData.CamType.FLASH, CamOutBaseData.Shape.Obround, innerLandDia, innerLandDia, pt0, pt1);    //L3は表層ランド形状に関係なくObround
                camOut.Add(camd);
            }

...
//			filename = Path.ChangeExtension(filename, "dump");
            ...
            //2018/10/27 内層PTHの接続情報を上のFillPolygonに依存しないようにした。ネストしたポリゴンで間違いが発生したことによる。
            ClearPTHinnerLayerConnectionInfo(mainList);
            SetPTHinnerLayerConnectionInfo2(mainList);
            //
        ...
        /// <summary>
        /// PTHのLineとポリゴンの構成線による内層接続フラグを設定する(CamOutでPTHの内層ランド有無判定に使う。
        /// </summary>
        /// <param name="objList"></param>
        protected void SetPTHinnerLayerConnectionInfo2(LinkedList<MbeObj> objList)
        {
            LinkedList<MbeObjPTH> pthList = new LinkedList<MbeObjPTH>();
            LinkedList<MbeObjLine> lineList = new LinkedList<MbeObjLine>();
            ...
            foreach (MbeObj obj in objList) {
                if (obj.DeleteCount < 0) {
                    ...
                    //ポリゴンの構成線の抽出   2018/10/27
                    else if (obj.Id() == MbeObjID.MbePolygon) {
                        if (obj.Layer == MbeLayer.LayerValue.L2 || obj.Layer == MbeLayer.LayerValue.L3) {
                            foreach(MbeGapChkObjLine fillLine in ((MbeObjPolygon)obj).fillLineList) {
                                //MbeObjLineに変換しないと使えない。
                                //ポリゴンの構成線の情報のうち必要なのは端点座標とレイヤーだけ。
                                MbeObjLine lineObj = new MbeObjLine();
                                lineObj.SetPos(fillLine.p0, 0);
                                lineObj.SetPos(fillLine.p1, 1);
                                lineObj.Layer = obj.Layer;
                                lineList.AddLast(lineObj);
                            }
                        }
                    }
                }
